...
export const Hero = () => {
  ...
  const slideShape1 = useTransform(scrollYProgress, [0, 1], ["0vh", "-50vh"]);

  const lenis = useLenis();
  const handleClick = () => {
    lenis?.scrollTo("#next-section", { offset: -50 });
  };

  return (
    <motion.div
      className="min-h-screen flex flex-col items-center justify-center bg-black sticky top-0 z-10 relative"
      ref={heroRef}
      style={{
        y: slideShape1,
      }}
    >
      ...
      <motion.button
        onClick={handleClick}
        className="mt-8 text-white opacity-75 hover:opacity-100 transition-opacity z-10 cursor-pointer"
        initial={{ y: 0 }}
        animate={{ y: [0, 10, 0] }}
        transition={{
          duration: 2,
